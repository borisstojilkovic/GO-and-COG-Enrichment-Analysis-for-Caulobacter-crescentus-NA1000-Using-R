...
generate_bubble_plot <- function(cog_results_enriched, file_name) {
  top_categories <- cog_results_enriched %>%
  ...
  top_categories$category_desc <- factor(
    paste(top_categories$category, top_categories$Description, sep = " - "),
    levels = rev(paste(top_categories$category, top_categories$Description, sep = " - "))
  )
  ...
  p <- ggplot(top_categories, aes(x = percent_DEG, y = category_desc, size = numDEInCat, color = over_represented_pvalue)) +
    ...
    labs(title = "Top 10 Enriched COG Categories", x = "% of DEG in category", y = "COG - Description", size = "DEG count") +
    theme_minimal() +
    theme(
  ...
  print(p)
  ggsave(filename = file.path(output_folder, paste0(tools::file_path_sans_ext(file_name), "_COG_bubble_plot.pdf")),
